        ...
        {activeTab.proxyUrl ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center relative p-6 overflow-y-auto">
            ...
            <div className="max-w-2xl w-full flex flex-col items-center text-center relative z-10 mt-8">
              ...
              {/* SEARCH BAR with fun fact placeholder */}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              {settingsTab==="history"&&(
                <div className="space-y-4">
                  <div className="space-y-2">
                    <div className="flex justify-between items-center"><span className="text-[9px] font-mono font-semibold tracking-wider text-zinc-500 uppercase">Search History</span>{searchHistory.length>0&&<button onClick={()=>{setSearchHistory([]);localStorage.removeItem("xena_search_history");}} className="text-[8px] font-mono uppercase bg-zinc-950 px-1.5 py-0.5 border border-zinc-850 hover:bg-zinc-900 text-zinc-400 hover:text-white rounded">Clear</button>}</div>
                    <div className="max-h-32 overflow-y-auto space-y-1 pr-1 no-scrollbar">{searchHistory.length===0?<div className="text-[10px] text-zinc-600 font-mono py-6 text-center border border-dashed border-zinc-900 rounded-lg">No search history.</div>:searchHistory.slice(0,20).map(item=>(<div key={item.id} className="flex items-center justify-between p-2 rounded border border-zinc-900 bg-black/40 hover:border-zinc-850"><div className="flex flex-col min-w-0 flex-1 mr-2 text-left"><span className="text-xs text-white truncate font-medium">{item.query}</span><span className="text-[8px] font-mono uppercase text-zinc-550 mt-0.5">{item.engine} • {item.timestamp}</span></div><button onClick={()=>{go(item.query);setModal(null);}} className="text-[9px] px-2 py-1 bg-zinc-900 border border-zinc-800 text-white rounded font-mono hover:bg-zinc-800">Search</button></div>))}</div>
                  </div>
                  <div className="space-y-2 pt-2 border-t border-zinc-800">
                    <div className="flex justify-between items-center"><span className="text-[9px] font-mono font-semibold tracking-wider text-zinc-500 uppercase">AI Usage History</span>{aiHistory.length>0&&<button onClick={()=>{setAiHistory([]);localStorage.removeItem("xena_ai_history");}} className="text-[8px] font-mono uppercase bg-zinc-950 px-1.5 py-0.5 border border-zinc-850 hover:bg-zinc-900 text-zinc-400 hover:text-white rounded">Clear</button>}</div>
                    <div className="max-h-32 overflow-y-auto space-y-1 pr-1 no-scrollbar">{aiHistory.length===0?<div className="text-[10px] text-zinc-600 font-mono py-6 text-center border border-dashed border-zinc-900 rounded-lg">No AI conversations yet.</div>:aiHistory.slice(0,15).map(item=>(<div key={item.id} className="p-2 rounded border border-zinc-900 bg-black/40"><div className="flex items-center gap-1.5 mb-1"><span className={`text-[8px] font-mono px-1 py-0.5 rounded ${item.mode==="serious"?"bg-emerald-900/30 text-emerald-400":"bg-purple-900/30 text-purple-400"}`}>{item.mode}</span><span className="text-[8px] text-zinc-500 font-mono">{item.timestamp}</span></div><p className="text-[10px] text-zinc-300 truncate">Q: {item.message}</p><p className="text-[10px] text-zinc-500 truncate">A: {item.response}</p></div>))}</div>
                  </div>
                </div>
              )}
              {settingsTab==="ddg"&&(
                <div className="space-y-3">
                  <span className="text-[9px] font-mono tracking-wider text-zinc-500 uppercase">Search Engine Mode</span>
                  <p className="text-[10px] text-zinc-500 font-mono">DuckDuckGo refuses iframe embedding. Try different modes:</p>
                  {Object.entries(DDG_OPTIONS).map(([k,o])=>(
                    <div key={k} onClick={()=>{setDdgMode(k);localStorage.setItem("xena_ddg_mode",k);}} className={`p-3 rounded-lg border cursor-pointer transition-all ${ddgMode===k?'bg-zinc-800 border-zinc-600':'bg-black border-zinc-800 hover:border-zinc-600'}`}>
                      <div className="flex items-center justify-between"><span className="text-xs font-semibold text-white">{o.name}</span>{ddgMode===k&&<span className="text-[8px] text-emerald-400 font-bold">✓ ACTIVE</span>}</div>
                      <p className="text-[9px] text-zinc-500 mt-1 font-mono">{o.note}</p>
                    </div>
                  ))}
                </div>
              )}
      ...
      {modal==="report"&&(
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center" onClick={()=>setModal(null)}>
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl w-full max-w-md p-6" onClick={e=>e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-2">
                <div><label className="block text-[9px] font-mono tracking-wider text-zinc-500 mb-1.5 uppercase">Kind</label><select value={reportForm.kind} onChange={e=>setReportForm({...reportForm,kind:e.target.value})} className="w-full h-9 rounded-lg border border-zinc-800 bg-black px-2 text-xs text-white focus:border-zinc-500"><option value="bug">🐛 Bug Report</option><option value="suggestion">💡 Suggestion</option></select></div>
                <div><label className="block text-[9px] font-mono tracking-wider text-zinc-500 mb-1.5 uppercase">Title</label><input type="text" value={reportForm.title} onChange={e=>setReportForm({...reportForm,title:e.target.value})} placeholder="Summary" className="w-full h-9 rounded-lg border border-zinc-800 bg-black px-3 text-xs text-white outline-none focus:border-zinc-500"/></div>
              </div>
              <div><label className="block text-[9px] font-mono tracking-wider text-zinc-500 mb-1.5 uppercase">URL (optional)</label><input type="text" value={reportForm.url} onChange={e=>setReportForm({...reportForm,url:e.target.value})} placeholder="https://..." className="w-full h-9 rounded-lg border border-zinc-800 bg-black px-3 text-xs text-white outline-none focus:border-zinc-500"/></div>
              <div><label className="block text-[9px] font-mono tracking-wider text-zinc-500 mb-1.5 uppercase">Details</label><textarea rows={4} value={reportForm.details} onChange={e=>setReportForm({...reportForm,details:e.target.value})} placeholder="What happened..." className="w-full rounded-lg border border-zinc-800 bg-black p-3 text-xs text-white outline-none resize-none focus:border-zinc-500"/></div>
